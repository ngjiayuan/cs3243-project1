...
class Board:
    def __init__(self, rows, cols, obs, obs_pos, enemies):
        self.rows = rows
        self.cols = cols
...
class Node:
    ...
    def transition(self, action):
        ...
        new_cost = 1 # cost irrelevant for BFS
...
def initialise():
    ...
    board = Board(rows, cols, obs, obs_pos, enemies)
    ...
    while len(frontier) != 0:
        ...
        for a in current.get_actions():
            ...
            if next_node.is_goal():
                return next_node.parents, nodes_explored
            if (next_node.piece.coord not in reached):
                frontier.append(next_node)
                reached.add(next_node.piece.coord) # graph-search      
